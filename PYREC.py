...
class PYREC(nn.Module):
    ...
    def build_hyper_edge(self, file):
        ...
        # Ensure hyperedge matrices match n_entities in size
        ...
        with open(file) as f:
            for l in f.readlines():
                ...
                # Populate interaction matrices, ensuring indices stay in range
        ...
        # Jaccard similarity for projects
        J_project = self.jaccard_similarity(project_library_interaction)
        project_indices = np.where(J_project > 0.50)  # Assuming alpha1=0.5
        project_values = J_project[project_indices]
        project_hyperedge_matrix = sp.coo_matrix((project_values, project_indices), shape=(PROJECT_NUM, PROJECT_NUM))
        self.project_hyperedge = project_hyperedge_matrix.T

        # Normalize project hyperedge matrix
        Dv_1, De_1 = self.get_D_inv(self.project_hyperedge)
        Dv_1 = self.sparse_mx_to_torch_sparse_tensor(Dv_1)
        De_1 = self.sparse_mx_to_torch_sparse_tensor(De_1)
        self.project_hyperedge = self.sparse_mx_to_torch_sparse_tensor(self.project_hyperedge)
        self.project_hyperedge_T = self.sparse_mx_to_torch_sparse_tensor(project_hyperedge_matrix)
        ...
        # Jaccard similarity for libraries
        J_library = self.jaccard_similarity(library_project_interaction)
        library_indices = np.where(J_library > 0.50)  # Assuming alpha2=0.5
        library_values = J_library[library_indices]
        library_hyperedge_matrix = sp.coo_matrix((library_values, library_indices), shape=(LIBRARY_NUM, LIBRARY_NUM))
        self.library_hyperedge = library_hyperedge_matrix.T

        # Normalize library hyperedge matrix
        Dv_1, De_1 = self.get_D_inv(self.library_hyperedge)
        Dv_1 = self.sparse_mx_to_torch_sparse_tensor(Dv_1)
        De_1 = self.sparse_mx_to_torch_sparse_tensor(De_1)
        self.library_hyperedge = self.sparse_mx_to_torch_sparse_tensor(self.library_hyperedge)
        self.library_hyperedge_T = self.sparse_mx_to_torch_sparse_tensor(library_hyperedge_matrix)

        spm1 = torch.sparse.mm(Dv_1, self.library_hyperedge)
        self.norm_lib1 = torch.sparse.mm(spm1, De_1)
